...
namespace Ch04 {
    namespace StencilTesting {
        ...
        int main(int argc, char * argv[]) {
            ...
            /*
                func：设置模板函数(Stencil Test Function)，这个测试函数将会应用到已存储的模板值上和glStencilFunc函数的ref值上
                ref：设置了模板测试的参考值(Reference Value)。模板缓冲的内容将会与这个值进行比较
                mask：设置一个掩码，它将会与参考值和存储的模板值在测试比较它们之前进行与(AND)运算。
            */
            ...
            */
            glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); // GL_REPLACE：将模板值设置为glStencilFunc函数设置的ref值
